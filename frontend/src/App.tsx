...
export const SharedSizeContext = createContext<{
  ...
  updateCardContent: (id: keyof CardConfigType, newContent: ReactNode) => void;
}>({
  cardSizes: {}, // Empty object as default
  updateCardSize: () => {}, // Empty function as default
  updateCardContent: () => {}, // Empty function as default
});
...
function App() {
  ...
  const updateCardContent = (
    ...
    // Update shared state with new size for the specific card
    setCardConfigs((prevConfig) => ({
      ...prevConfig,
      [id]: { ...prevConfig[id], content: newContent },
    }));
  };

  return (
    <SharedSizeContext.Provider
      value={{ cardSizes: cardConfigs, updateCardSize, updateCardContent }}
    >
      <main>
